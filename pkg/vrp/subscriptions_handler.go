        ...
        if sub.NotifyForOperationalIntents {
            ...
            for _, op := range relevantOperations {
                if op.Manager != manager {
                    op.OVN = vrpmodels.OVN(scdmodels.NoOvnPhrase)
                }
                pop, _ := op.ToProto()
...
func (a *Server) DeleteSubscription(ctx context.Context, req *vrppb.DeleteVertiportSubscriptionRequest) (*vrppb.DeleteVertiportSubscriptionResponse, error) {
    // Retrieve VertiportSubscription ID
    id, err := dssmodels.IDFromString(req.GetSubscriptionid())
    if err != nil {
