...
from monitoring.monitorlib import scd    # for scd.NO_OVN_PHRASES
...
# Delete Operations and Subscription if exist
# Preconditions: None
# Mutations: Operations and Subscription deleted if exist
...
# Preconditions: None
# Mutations: None
def test_op1_does_not_exist_get_1(ids, vrp_session, vrp_session2):
...
# Preconditions: None
# Mutations: None
def test_op1_does_not_exist_get_2(ids, vrp_session2):
...
# Preconditions: None
# Mutations: None
def test_op1_does_not_exist_query_1(ids, vrp_session, vrp_session2):
...
# Preconditions: None
# Mutations: None
def test_op1_does_not_exist_query_2(ids, vrp_session, vrp_session2):
...
def test_create_op1(ids, vrp_session, vrp_session2):
...
# Mutations: None
def test_delete_implicit_sub(ids, vrp_session, vrp_session2):
  if vrp_session is None:
    return
  resp = vrp_session.get('/operational_intent_references/{}'.format(ids(OP1_TYPE)), scope=SCOPE_VRP)
  assert resp.status_code == 200, resp.content
...
# Preconditions: Operation Op1 created by vrp_session user
# Mutations: None
def test_delete_op1_by_uss2(ids, vrp_session, vrp_session2):
...
# Preconditions: Operation Op1 created by vrp_session user
# Mutations: None
def test_create_op2_no_sub(ids, vrp_session, vrp_session2):
...
# Mutations: None
def test_create_op2_no_key(ids, vrp_session, vrp_session2):
  req = _make_op2_request()
...
def test_create_op2(ids, vrp_session, vrp_session2):
  req = _make_op2_request()
  req['subscription_id'] = ids(SUB2_TYPE)
  req['key'] = [op1_ovn]
  resp = vrp_session2.put('/operational_intent_references/{}'.format(ids(OP2_TYPE)), json=req, scope=SCOPE_VRP)
...
# Mutations: None
def test_read_ops_from_uss1(ids, vrp_session, vrp_session2):
  if vrp_session is None:
    return
...
#   * Operation Op2 created by vrp_session2 user
# Mutations: None
def test_read_ops_from_uss2(ids, vrp_session, vrp_session2):
  if vrp_session2 is None:
    return
  time_now = datetime.datetime.utcnow()
  end_time = time_now + datetime.timedelta(hours=1)
  resp = vrp_session2.post('/operational_intent_references/query', 
    json = {
        'vertiport_reservation_of_interest': {
                'time_start': make_time(time_now),
                'time_end': make_time(end_time),
...
#   * Operation Op2 created by vrp_session2 user
# Mutations: None
def test_mutate_op1_bad_key(ids, vrp_session, vrp_session2):
  resp = vrp_session.get('/operational_intent_references/{}'.format(ids(OP1_TYPE)), scope=SCOPE_VRP)
  assert resp.status_code == 200, resp.content
...
def test_mutate_op1(ids, vrp_session, vrp_session2):
  resp = vrp_session.get('/operational_intent_references/{}'.format(ids(OP1_TYPE)), scope=SCOPE_VRP)
  assert resp.status_code == 200, resp.content
  existing_op = resp.json().get('operational_intent_reference', None)
  assert existing_op is not None, resp.content
...
def test_delete_op1(ids, vrp_session, vrp_session2):
  resp = vrp_session.delete('/operational_intent_references/{}/{}'.format(ids(OP1_TYPE), op1_ovn), scope=SCOPE_VRP)
  assert resp.status_code == 200, resp.content
...
def test_delete_op2(ids, vrp_session, vrp_session2):
  resp = vrp_session2.delete('/operational_intent_references/{}/{}'.format(ids(OP2_TYPE), op2_ovn), scope=SCOPE_VRP)
  assert resp.status_code == 200, resp.content
...
# Ensure Operations and Subscription do not exist
# Preconditions: none
# Mutations: Operations and Subscription deleted if exist
